/* decor.h: theme engine (flat / classic / simple / love / win95) */
 ...
 * background (all themes but "simple", which never shows a title bar), and
 * the button glyphs for themes that have them, based on the configured
 * theme and the window's decor_title/decor_border flags. Also refreshes
 ...
 * outer rectangle, decorations included, not just the content area. Used
 * to stop a hit-test walk at the topmost window covering a point, since
 * decor_hit_test() alone cannot tell "not my decoration" apart from "not
 * my window at all". */
